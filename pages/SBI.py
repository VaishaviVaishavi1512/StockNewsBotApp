...
CURRENT_STOCK = "SBI"
FULL_STOCK_NAME = "State Bank of India" # Added for better display
...
def perform_ner(text, current_stock_symbol, full_stock_name):
    ...
    # Consolidated, specific aliases and strongly associated sector keywords for the CURRENT_STOCK (SBI)
...
# Fetch news specifically for SBI
# Changed query for broader initial retrieval, then filter with NER
raw_articles = get_financial_news_api(f'"{FULL_STOCK_NAME}" OR "{CURRENT_STOCK}" State Bank of India OR SBI stock India banking')
...
if raw_articles:
    ...
    for news_item in raw_articles:
        ...
        # Use the refined NER to check if the article is specifically about SBI
        ticker_identified = perform_ner(full_text, CURRENT_STOCK, FULL_STOCK_NAME)
        
        if ticker_identified == CURRENT_STOCK: # Only process if it's explicitly about SBI
